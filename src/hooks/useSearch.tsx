...
export default function useSearch() {
  ...
  const [isTitleMessageVisible, setIsTitleMessageVisible] =
    useState<boolean>(false);
  const [suggestions, setSuggestions] = useState<TResults>([]);
  const [isSuggestionsOpen, setIsSuggestionsOpen] = useState<boolean>(false);
  const titleToSearch: string = searchTitle
    .toLowerCase()
    .trim()
    .replace("&", "%2526");
  ...
  function updateSearchTitleOnChange(e: ChangeEvent) {
    ...
    const isSearchBarEmpty: boolean =
      searchBarValue.trim().split("").length <= 0;
  ...
  const handleSearchFormSubmit: THandleSearchFormSubmit = (
    e,
    navigate,
    setSearchParams,
  ) => {
    ...
    if (isSearchBarEmpty) {
      ...
      navigate
        ...
        : setSearchParams
          ? setSearchParams({ title: titleToSearch })
          : "";
    }
  };
