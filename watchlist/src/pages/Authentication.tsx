...
export default function Authentication(): JSX.Element {
  ...
  function deleteUserWatchlistData() {
    ...
    const promises = userWatchlist.map((film) =>
      deleteDoc(doc(db, "watchlist", film.docId)),
    );
    setUserWatchlist(() => []);
    ...
    <div className="w-screen min-h-screen bg-authBg bg-cover bg-center bg-red-800 bg-blend-overlay">
      <div className="flex justify-center items-center w-screen min-h-screen backdrop-blur-sm overflow-y-auto">
        <section className="flex flex-col items-center w-[80%] md:w-[30%] rounded-lg bg-zinc-900/80 px-8 py-8">
          <h1 className="self-center font-robotoCondensed text-4xl text-slate-50 font-semibold leading-none">
            {isDeleteAccount
              ? "Sad to see you go!"
              : `Welcome${isSignIn ? " back" : ""}!`}
          </h1>
          <p className="self-center font-inter mt-2 mb-8 text-base text-zinc-400">
            {isSignIn
              ? "Sign in to access your watchlist."
              : isDeleteAccount
                ? "Confirm account ownership before deleting."
                : "Sign up to add movies to your watchlist."}
          </p>
          <form
          ...
          >
            <fieldset className="relative flex flex-col gap-6 w-full">
              <input
                className="box-border h-10 leading-none p-2 text-zinc-300 font-inter bg-transparent outline-none border border-zinc-300 rounded-lg"
                type="text"
              ...
              />
              <input
                className={`${PASSWORD_INPUT_BORDER_CLASS} box-border h-10 leading-none p-2 text-zinc-300 font-inter bg-transparent outline-none border rounded-lg`}
                type="password"
            ...
            <button
              className={`${loading ? "cursor-not-allowed" : ""} grid place-content-center box-border h-12 leading-none w-full mt-10 py-2 bg-red-800 hover:bg-red-900 text-slate-50 font-inter font-semibold rounded-lg transition-all active:translate-y-1`}
          ...
          {!isDeleteAccount && (
            <p className="text-sm font-light font-inter mt-8 self-center text-slate-50">
              ...
              <Link
                ...
                replace={true}
                className="text-red-700 text-base font-semibold hover:underline hover:underline-offset-2"
              >
