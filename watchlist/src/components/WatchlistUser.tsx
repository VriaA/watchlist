...
export default function WatchlistUser({ style }: TWatchlistUser) {
  ...
  const { isLoggedIn, signedInUser, auth, setDialog, openDialog } = useContext(
    AppContext,
  ) as TAppContext;
  ...
  return (
    ...
    >
      {loading && (
        <Loader style="fixed top-0 left-0 w-screen h-screen z-[9999]" />
      )}
        ...
        >
          ...
          {isLoggedIn && (
            ...
            >
              ...
              <svg
                className="fill-zinc-100 group-hover:fill-red-500 "
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                viewBox="0 0 448 512"
      ...
      <dialog
        className={`${isDeleteDialogOpen ? "flex" : ""} flex-col gap-6 items-start w-[70%] max-w-[300px] p-5 backdrop:bg-zinc-900/40 font-inter rounded-lg`}
        ref={deleteDialogRef}
      >
        ...
        <div className="flex justify-end self-end gap-4">
          <Link
            to="../delete-account"
            className="w-20 text-center bg-red-800 text-zinc-100 font-semibold tracking-wide rounded-lg px-[.5em] py-[.25em]"
          >
            Yes
          </Link>
          <button
            className="w-20 text-center border border-zinc-900 text-zinc-900 font-semibold tracking-wide rounded-lg px-[.5em] py-[.25em]"
