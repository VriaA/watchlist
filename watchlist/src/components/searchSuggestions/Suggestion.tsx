...
export default function Suggestion({suggestion}: {suggestion: TResult}) {
    const {id, media_type, type, title, name, first_air_date, release_date, poster_path} = suggestion
    const date = first_air_date || release_date
    const filmTitle = title || name

    return  (
                <li title="Suggestion">
                    <Link to={`/${media_type||type}/${id}`} className="max-w-[90%] flex gap-2 text-xs md:text-sm">
