...
export default function SavedFilmPoster({ film }: { film: DocumentData }): JSX.Element {
    ...
    return (
        <>
            {
                poster ?
                    <Link to={`/${filmType}/${name}+${filmId}`} className="grid grid-cols-1 grid-rows-1 min-h-[200px] max-h-[280px]">
                        <img className="col-start-1 row-start-1 z-[8] w-full h-full object-cover object-center rounded-t-md cursor-pointer" src={poster} alt={name} title={name} loading="lazy"></img>
                        <span className="block col-start-1 row-start-1 z-[7] w-full h-full bg-zinc-900/40 animate-pulse"></span>
                    </Link>
                    :
                    <Link to={`/${filmType}/${name}+${filmId}`} className="flex items-center justify-center min-h-[200px] max-h-[280px] bg-zinc-800/40 cursor-pointer">
                        <svg className="w-12 h-12 lg:w-[72px] lg:h-[72px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="#ffffff ">
