...
export default function SavedFilm({
  ...
  const { setDialog, openDialog, setLoading } = useContext(
    AppContext,
  ) as TAppContext;
    ...
    >
      {iswatched && (
        <div
          ...
          aria-label={`${name} marked as watched`}
        >
          <svg
            className="w-5 lg:w-6 aspect-square drop-shadow-blackOutline"
        ...
        <div className="flex justify-between items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            <p className="flex items-center gap-1">
              {getRunTime(runtime)} mins
            </p>
          </div>
          <p className="px-2 py-[2px] bg-zinc-900/40 backdrop-blur-sm border rounded-md">
            {FilmType(filmType)}
          </p>
        </div>
      </div>
