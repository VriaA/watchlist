...
export default function Videos({videos}: {videos: {results: TVideo[]}}) {
    ...
    function Video() {
        if(videos.results.length === 0) {
            return (
                    <p className="card-videos w-full h-full text-lg lg:text-xl font-light text-center">
                        No videos.
                    </p>
                )
        }
        return videos.results.map(video=> {
             const {id, key} = video
            return (<iframe className="card card-videos snap-start flex-none w-[100%] lg:w-[50%] h-80 rounded-md"
