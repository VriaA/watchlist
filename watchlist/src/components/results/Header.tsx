  ...
  return (
    <header ref={headerRef} className="w-full h-fit grid grid-cols-3 grid-rows-2 lg:grid-cols-4 lg:grid-rows-1 items-center gap-2 md:gap-4 mt-4 -translate-y-16 opacity-0">
      ...
      >
        ...
        <span
          className={`${isTitleMessageVisible ? "opacity-1" : "opacity-0"} absolute z-50 block w-fit inset-0 mr-auto ml-auto mt-auto h-fit -mb-8 md:mb-[-36px] px-2 md:px-4 py-1 text-[8px] min-[375px]:text-xs md:text-sm bg-zinc-900 font-normal leading-wide text-slate-50 rounded-full transition-opacity`}
        >
