...
export type TUseWatchlist = {
  ...
  setUserWatchlist: React.Dispatch<
...
export default function useWatchlist({
  ...
  signedInUser,
  setLoading,
}: TUseWatchlistParams): TUseWatchlist {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      try {
        let watchlist: DocumentData[] = snapshot.docs.map((doc) => ({
          ...doc.data(),
          docId: doc.id,
        }));
  ...
  function getFilmInWatchlist(
    film: TFilmInWatchlist,
  ): DocumentData | undefined {
    if (userWatchlist && userWatchlist.length > 0) {
      return userWatchlist.find(
        (filmInWatchlist) => filmInWatchlist.filmId === film.filmId,
      );
    }
  }
